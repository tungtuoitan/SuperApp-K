# Setup Debug Mobile — chrome://inspect, Port Forwarding, ADB

## Có mấy cách debug mobile với SuperApp?
2 cách: USB + port forwarding, hoặc cùng WiFi + local IP.
## Khi remote-debugging bằng cable thì packet đi: mobile Chrome → cable → PC Chrome → webpack-dev-server. WiFi thì: mobile Chrome → router → PC Chrome → webpack-dev-server. Đúng không?
Gần đúng nhưng "PC Chrome" không tham gia truyền data — Chrome trên PC chỉ để inspect (xem DOM, console), không làm relay.
- **Cable**: mobile Chrome → ADB daemon trên PC (qua USB) → `localhost:3000` → webpack-dev-server
- **WiFi**: mobile Chrome → WiFi router → WiFi adapter của PC → webpack-dev-server (thẳng vào, không qua Chrome PC)
## Công cụ nào dùng để debug mobile?
`chrome://inspect/#devices` trên laptop.

## Daemon là gì? Đọc là gì?
...
## Daemon khác gì job?
- **Daemon**: chạy ngầm **liên tục**, luôn sẵn sàng, không kết thúc. Giống "bảo vệ trực ca" — chờ có người gọi thì phục vụ. Ví dụ: ADB daemon luôn chạy, chờ phone kết nối USB.
- **Job**: chạy **một lần** rồi kết thúc — có đầu vào, xử lý, done. Ví dụ: cron job backup database lúc 2h sáng, chạy xong là nghỉ.
...
## 1 port chỉ bind được 1 process, nhưng 1 process bind được nhiều port?
...
## IP và host có nghĩa tương tự nhau không?
...
## USB cable có phải network interface không?
...
## Nếu PC kết nối Ethernet, mobile kết nối WiFi thì chrome://inspect không dùng được?
...
## BE cần sửa file nào để nhận connection từ ngoài (WiFi)?
`launchSettings.json` — đổi `applicationUrl` từ `http://localhost:5000` thành `http://0.0.0.0:5000`.

## Khi forward, phải có app nào đó nhận và forward request?
...
## Port forwarding hoạt động thế nào ở tầng thấp?
...
## Tại sao API pending khi dùng WiFi IP nhưng FE vào được bình thường?
...
## Firewall chỉ áp dụng cho wireless, cable thì không bị check?
...
## Khi thiết bị A truy cập B:3000, nghĩa là gửi 1 packet?
...
Cả hai đều không phải. Interface là lớp phần mềm/driver. Cable và cổng là phần cứng vật lý. ADB driver tạo ra virtual interface sử dụng USB làm phương tiện truyền tải bên dưới.

## `applicationUrl` chính là "lỗ tai" của app?
...
## Host có nghĩa là máy vật lý không?
Trong networking, "host" = bất kỳ thiết bị nào có IP trên mạng (laptop, phone, server). Nhưng trong config như `host: "0.0.0.0"` hay `WDS_SOCKET_HOST`, "host" không chỉ máy vật lý — nó chỉ IP/địa chỉ mà process bind hoặc kết nối tới.
