...
<!-- # việc quay lại main thread hay thread bất kì sau await thì có gì quan trọng không?
...
# lí do thread pool tồn tại là để tiết kiệm overhead do bỏ qua việc tạo/dọn phải không?
Đúng. 
Tạo OS thread mới tốn ~1ms và 1MB stack — pool giữ sẵn 1 nhóm thread, tái sử dụng cho nhiều task ngắn. Tránh tạo/dọn liên tục.
# tạo 1 OS thread mới thì tốn gì?
~1ms và 1MB stack
# liên hệ giữa stack và pool.thread?
Mỗi pool thread có 1 stack riêng 
(~1MB) cấp khi thread tạo ra. 
# khi nào stack được reset? 
khi 1 task kết thúc
# thread chạy 1 task tại 1 thời điểm à?
Đúng. 
...
# pool.thread khác .NET Thread ở chỗ pool.thread luôn sống có phải không?
Đúng. 
...
# hoạt động của scheduler, queue, thread pool?
khi có thread rảnh, scheduler bốc task trong queue ra cho thread chạy
# scheduler, queue, thread pool đều thuộc runtime phải không?
Đúng. 
...
# mọi hoạt động trong .net đều dùng pool.thread trừ khi dùng .NET thread phải không
Gần đúng. 
...
# công việc của pool.thread chỉ là xử lí task từ queue phải không?
Đúng. 
...
# worker liên hệ gì với pool.thread?
