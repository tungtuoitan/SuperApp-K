...
# các cách triển khai fire-and-forget? và khi nào thì dùng cách nào? [id:3652 order:5]
...
# CPU-bound work là gì? [id:2817 order:8]
...
# "abstraction cao hơn" nghĩa là gì? cho ví dụ phổ biến [id:2820 order:12]
là layer nằm trên, ẩn đi chi tiết của layer bên dưới
Ví dụ: Task ẩn Thread, EF Core ẩn SQL, HttpClient ẩn TCP socket.

# abstraction có những nghĩa nào, là loại từ gì? [id:2821 order:13]
...
# Thread .NET có liên hệ gì với OS thread? [id:2900 order:19]
...
# method trả về Task thì nói lên điều gì? [id:2951 order:20]
```cs
...
# result type Task có nghĩa là chưa có kq, đang đợi có phải không? [id:2952 order:21]
Đúng.
...
# string dạng này gọi là gì? $"{x}-abc"; [id:2956 order:25]
là string interpolation

# các modifier phổ biến? [id:2957 order:26]
