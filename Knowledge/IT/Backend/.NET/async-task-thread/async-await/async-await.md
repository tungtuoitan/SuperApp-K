...
# các trường hợp làm task complete nhanh đến nỗi thread k kịp yield? [id:3416 order:18]
Khi Task đã done trước khi `await` được evaluated"
- cache hit,
- I/O xong ngay trong cùng time slice.
- trong async k có await thật

# hầu hết IO-bound sẽ kịp yield phải không? [id:3417 order:19]
đúng

# ta có thể biết chính xác được cái nào chạy trước hết phải không? task complete vs thread yield [id:3418 order:20]
Không.
Lúc viết code không biết trước — phụ thuộc runtime: cache hit hay miss, network nhanh hay chậm, OS schedule thế nào.

# thứ tự task complete/thread yield có ảnh hưởng kq không? vì sao? [id:3419 order:21]
...
# không kịp yield nghĩa là gì? [id:3420 order:22]
là Task hoàn thành QUÁ NHANH, cho nên thread sẽ chạy tiếp luôn

# thread gốc/main thread là gì? [id:3421 order:23]
là request thread hoặc UI thread tùy ngữ cảnh

# batch nghĩa là gì? [id:3400 order:24]
lô / nhóm
...
# stream nghĩa là gì? [id:3402 order:25]
...
# IAsyncEnumerable có phổ biến k, được dùng trong trường hợp nào [id:3403 order:26]
không phổ biến
Dùng khi stream, đọc file lớn

# khi nào dùng IAsyncEnumerable? [id:3404 order:27]
khi cần stream dữ liệu lớn mà không muốn load hết vào memory.

# có nên dùng async void không? vì sao? [id:3469 order:28]
Không.
vì k trả về Task thì k thể xử lí tiếp được, do đó k thể await và exception bị nuốt

# có cần quan tâm SynchronizationContext không? vì sao? [id:3470 order:29]
không
vì Trong ASP.NET Core: **không có** `SynchronizationContext`
— sau `await`, continuation chạy trên pool thread bất kỳ.

# việc capture thì tốn tài nguyên à? [id:3471 order:30]
...
# trong ASP.NET Core hiện đại,,khi nào thì cần dùng ConfigureAwait(false) ? [id:3472 order:31]
k cần dùng,
vì ASP.NET Core không có `SynchronizationContext`.

<!--# trong ASP.NET Core hiện đại, các lỗi sai thường gặp đi dùng async await? [id:3473 order:32]
...
# code này có vấn đề gì? [id:3474 order:33]
```cs
...
<!--# parallel.ForEach là gì? có phổ biến không? khi nào dùng? khác gì ForEach thông thường? [id:3475 order:34]
...
# k nên dùng async await bên trong parallel.ForEach phải không? vì sao? [id:3476 order:35]
Đúng.
`Parallel.ForEach` không hiểu `async` lambda — chạy như `async void`, không await được.

<!--# tại sao có Task.WhenAll() rồi mà vẫn cần parallel.ForEach? [id:3477 order:36]
...
# Task.WhenAll() hoạt động thế nào? [id:3478 order:37]
khi tất cả Task complete mới trả về.

<!--# có dùng số thread = số task không? [id:3479 order:38]
Số thread phụ thuộc vào Task bên trong: I/O-bound có thể không cần thread nào lúc chờ. -->

<!--# Task.WhenAll(multiple Task.Run( CPU-bound work)) có tương tự Parallel.ForEach không? [id:3480 order:39]
...
# tại sao parallel.foreach() k hợp với async await? [id:3481 order:40]
...
# có vấn đề gì? vì sao? [id:3482 order:41]
```cs
try
{
...
# lưu ý khi dùng semaphoreslim? [id:3485 order:44]
- Luôn `Release()` trong `finally`

# nếu quên release() trong semaphoreslim thì sao? [id:3486 order:45]
...
# Semaphore luôn hoạt động nhờ count à? [id:3487 order:46]
...
# dispose là gì? [id:3488 order:47]
...
# trong ASP.NET Core hiện đại, .Result và .Wait có được dùng phổ biến không? vì sao? [id:3490 order:49]
Không.
vì chúng Vẫn block thread > gây lãng phí thread pool.

# vấn đề gì? [id:3491 order:50]
```cs
...
# tại sao await trong lock lại bị compile error? [id:3493 order:52]
...
# dùng `await foreach` được à? [id:3494 order:53]
Được. Dùng với `IAsyncEnumerable<T>`.

<!--# khi nào cần dùng await foreach? [id:3495 order:54]
...
<!--# lí do await foreach tồn tại là cho phép xử lí dữ liệu từng phần theo cách bất đồng bộ, để không block thread phải k? [id:3496 order:55]
Đúng. -->

<!--# await foreach hoạt động thế nào? [id:3497 order:56]
Mỗi iteration gọi `MoveNextAsync()` trên enumerator — `await` chờ phần tử tiếp theo sẵn sàng rồi tiếp tục. -->

<!--# await foreach khác foreach thông thường ở chỗ hoạt động load item sẽ k block thread nữa , có phải không? [id:3498 order:57]
Đúng.
...
# `await foreach` thường dùng làm gì? [id:3499 order:58]
- stream data từ DB, API, file lớn.

# pitfall là gì? [id:3500 order:59]
