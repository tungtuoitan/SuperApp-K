...
# khi chạy await async() thì khi nào thread thực sự được giải phóng?
khi chạy đến await đầu tiên trong async()

# yield là gì?
là trả thread về pool

# khi nào thread yield?
Khi `await` một Task chưa complete

# khi nào thread k kịp yield ?
khi task complete quá nhanh

# các trường hợp làm task complete nhanh đến nỗi thread k kịp yield?
Khi Task đã done trước khi `await` được evaluated"
- cache hit,
- I/O xong ngay trong cùng time slice.
- trong async k có await thật

# hầu hết IO-bound sẽ kịp yield phải không?
đúng

# ta có thể biết chính xác được cái nào chạy trước hết phải không? task complete vs thread yield
Không. 
Lúc viết code không biết trước — phụ thuộc runtime: cache hit hay miss, network nhanh hay chậm, OS schedule thế nào.

# thứ tự task complete/thread yield có ảnh hưởng kq không? vì sao?
