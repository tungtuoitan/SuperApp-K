...
# Điểm mạnh của ASP.NET Core là gì? [id:2746 order:6]
- Cross-platform
- High performance

# High performance trong ASP.NET Core nghĩa là gì? [id:2747 order:7]
...
# Vì sao ASP.NET Core nhanh? [id:2748 order:8]
...
# Vì sao ASP.NET Core có thể cross-platform? [id:2749 order:9]
...
# Điều kiện để app cross-platform là gì? [id:2750 order:10]
Cần 3 điều kiện:
...
# this luôn có nghĩa là tham chiếu instance phải không? [id:2960 order:12]
đúng

# param của constructor đến từ đâu? [id:2961 order:13]
Từ params khi gọi `new` X(...)

# trong constructor thì khi nào cần dùng this? [id:2962 order:14]
...
# vai trò lớn nhất của this trong thực tế? [id:2963 order:15]
Phân biệt field với tham số trùng tên trong constructor.

# this được dùng và không được dùng ở đâu? [id:2965 order:16]
`this` dùng được ở mọi instance method/property.
không dùng được trong `static` method

# vì sao this không được dùng trong static method? [id:2966 order:17]
vì static không thuộc instance nào.

# Các cách dùng this phổ biến và ví dụ? [id:2967 order:18]
...
# khi intance khởi tạo, constructor sẽ chạy trước hay sẽ khởi tạo field trước? [id:2978 order:30]
