...
# tại sao có const rồi mà vẫn cần readonly? [id:3235 order:6]
...
# tại sao const cần biết value lúc compile time? [id:3236 order:7]
vì với const, compiler dùng value trực tiếp vào code

# tại sao const chỉ được dùng với primitive/string? [id:3237 order:8]
...
# c#.const được dùng cho member nào? [id:3241 order:12]
...
# tại sao const k được dùng cho property, method? [id:3242 order:13]
vì property và method cần runtime

# tại sao nên dùng public const string Blue = "blue"; thay vì public string Blue = "blue"; [id:3243 order:14]
để code rõ nghĩa hơn, vì biến này là constant

# readonly đi cùng set được không? vì sao? [id:3244 order:15]
Không.
vì chúng có ý nghĩa trái ngược: readonly là immutable, còn set là mutable

# sự khác nhau giữa init và set? [id:3245 order:16]
`set` cho phép gán bất kỳ lúc nào.
`init` chỉ cho phép gán lần đầu

# sealed class là gì? [id:3246 order:17]
là class không thể bị kế thừa.

# khi nào dùng sealed? [id:3247 order:18]
...
# sealed giúp bảo mật khỏi hacker à? [id:3248 order:19]
...
<!--# lí do sealed tồn tại? [id:3249 order:20]
để nói với dev rằng k nên extend class -->

# extend khác gì kế thừa? [id:3250 order:21]
Cùng nghĩa.
