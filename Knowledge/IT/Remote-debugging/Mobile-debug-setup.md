...
# deamon, job có thể là process cũng có thể là class à?
...
# `applicationUrl` trong `launchSettings.json` là gì? [id:539 order:7]
...
# `chrome://inspect/#devices` hoạt động thế nào? [id:540 order:8]
...
# Mỗi app thường chỉ có 1 port để nhận request thôi? [id:541 order:9]
...
# 1 port chỉ bind được 1 process, nhưng 1 process bind được nhiều port? [id:542 order:10]
...
# IP và host có nghĩa tương tự nhau không? [id:543 order:11]
...
<!--# Khi dùng port forwarding, phone truy cập FE bằng URL nào? [id:544 order:12]
`http://localhost:3000`. -->

<!--# Tại sao port forwarding không cần cùng mạng? [id:545 order:13]
...
<!--# Nếu PC kết nối Ethernet, mobile kết nối WiFi thì chrome://inspect không dùng được? [id:547 order:15]
...
# BE cần sửa file nào để nhận connection từ ngoài (WiFi)? [id:548 order:16]
`launchSettings.json` — đổi `applicationUrl` từ `http://localhost:5000` thành `http://0.0.0.0:5000`.

# Khi forward, phải có app nào đó nhận và forward request? [id:549 order:17]
...
<!--# Port forwarding hoạt động thế nào ở tầng thấp? [id:550 order:18]
...
<!--# Tại sao API pending khi dùng WiFi IP nhưng FE vào được bình thường? [id:551 order:19]
...
# Firewall chỉ áp dụng cho wireless, cable thì không bị check? [id:552 order:20]
...
# API pending có nghĩa là gì? [id:553 order:21]
...
# Khi thiết bị A truy cập B:3000, nghĩa là gửi 1 packet? [id:554 order:22]
...
# Chuyện gì xảy ra khi phone gõ `192.168.2.26:3000`? [id:555 order:23]
