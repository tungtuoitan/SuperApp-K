...
# user trao quyền cho web vào gg Drive, so với Azure trao quyền cho user thì khác nhau gì? [id:3632 order:2]
...
# user trao quyền cho web vào gg Drive, so với Azure trao quyền cho user thì khác nhau gì? [id:3617 order:3]
khác về **ai trao cho ai**:
- Drive: **user** trao quyền cho **web/app** truy cập tài nguyên của chính user (delegated, OAuth consent).
- Azure: **admin/tenant** trao quyền cho **user** truy cập tài nguyên của tổ chức (assigned, RBAC).

# ví dụ về Uỷ quyền? [id:38 order:4]
...
# Uỷ quyền và trao quyền có phải là 1 không? [id:39 order:5]
có, chúng là 1.

# Phân biệt Authentication và authorization? [id:40 order:6]
- authentication là xác thực, còn authorization là trao quyền
- E trước O
- xác thực xong thì mới trao quyền

# Token-based Authentication là gì? [id:61 order:7]
- là xác thực dựa trên Token

# Authentication là gì? [id:64 order:8]
